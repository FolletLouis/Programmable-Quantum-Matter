...
m_val = [10**2, 10**3, 10**4]             # multiplier for B sequence
...
colors = plasma(np.linspace(0, 0.7, 4))
m_ind = 0
...
for m in m_val:
    ...
    # Plot on left y-axis
    #ax1.plot(N_values, T2B_mod_array * 1e6, 'o--', color=colors[m_ind], label=fr'm = {m}', linewidth=1.8)
    ax1.plot(N_values, 1e3*Temp_Siv_B, 'o--', color=colors[m_ind], label=fr'm = {m}', linewidth=1.8)
    #ax1.tick_params(axis='y', labelcolor=color1)
    #ax1.yaxis.label.set_color(color1)
    ax1.set_xlabel("N", fontsize=28)
    # ax1.set_ylabel(r"$T_{2B}$ ($\mu$s)", fontsize=28)
